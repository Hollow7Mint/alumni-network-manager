"""Alumni Network Manager — router for event payloads."""
...
class AlumniRouter:
    """Router for Alumni Network Manager event payloads."""
...
def parse_events(payload: str) -> List[Dict[str, Any]]:
    ...
    return [AlumniRouter._coerce(item) for item in raw]


def archive_event_to_str(
    record: Dict[str, Any], indent: Optional[int] = None
) -> str:
    """Convenience wrapper — serialise a Event to a JSON string."""
    if indent is None:
        return AlumniRouter.dumps(record)
